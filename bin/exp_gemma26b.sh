#!/usr/bin/env bash
set -uo pipefail
# exp_gemma26b.sh — gemma4:26b (local, 262K) abstract zero_shot vs prompt+skills
# 262K context → can use FULL skill set via filter_skills.sh

cd ~/src/autotvb

BATCH_ROOT="sandbox/gemma26b_$(date +%Y%m%d_%H%M%S)"
GEN_MODEL="ollama/gemma4:26b"
...
mkdir -p "$BATCH_ROOT"
echo "=== gemma4:26b Experiment (262K context, full skills) ==="
echo "Gen:  $GEN_MODEL"
